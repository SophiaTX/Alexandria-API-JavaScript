...
sophia.getAccountHistoryByType = function (accountName, type, from, limit, callback) {
    return sophia.call('alexandria_api.get_account_history', { account: accountName, from: from, limit: limit }, function (err, response) {
        if (err) callback(err, '');else {
        ...
        if (err) callback(err, '');else {
            if (response.length > 0) {
                response.forEach(function (r) {
                    ...
                    var result = '{ "app_id":"' + r[r.length - 1].app_id + ',' + '                          "binary": "' + r[r.length - 1].binary + '",' + '                         "recipients": "' + r[r.length - 1].recipients + '",' + '                     "sender": "' + r[r.length - 1].sender + '",' + '                     "data": ' + simplifieddata + ',' + '                    "received": "' + r[r.length - 1].received + '"}';
                    var objectResult = JSON.parse(result);
                    callback('', objectResult);
                });
            } else {
