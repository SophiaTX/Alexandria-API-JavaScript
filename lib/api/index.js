                            ...
                            if (err) callback(err, '');else {
                                ...
                                _this2.call('add_fee', [operation, response], function (err, response) {
                                    if (err) {
                                        ...
                                        _this2.call('get_transaction_digest', [operationWithFee, chain_id], function (err, response) {
                                            if (err) callback(err, '');else {
                                                var sign = auth.createSignature(response, private_key);

                                                _this2.call('add_signature', [operationWithFee, sign], function (err, response) {
                                                    if (err) callback(err, '');else {
