...
var Sophia = function (_EventEmitter) {
    ...
    _createClass(Sophia, [{
            ...
            try {
                return this.call('about', [''], function (err, response) {
                    if (err) callback(err, '');else {
                        ...
                        _this2.call('get_transaction_digest', [operation, response.chain_id], function (err, response) {
                            if (err) callback(err, '');else {
                                var sign = auth.createSignature(response, private_key);

                                _this2.call('add_signature', [operation, sign], function (err, response) {
                                    if (err) callback(err, '');else {
        ...
        */
        ...
        key: 'updateAccount',
        value: function updateAccount(account_name, private_key, json_meta, owner, active, memo_key, callback) {
            var _this4 = this;
        ...
        key: 'voteForWitness',
        value: function voteForWitness(witness_to_vote_for) {
