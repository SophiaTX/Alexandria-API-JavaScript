...
sophia.getGroup = function (groupName, callback) {
    return groups.findOne({ 'group_name': groupName }, function (err, obj) {
                ...
                var _loop = function _loop(i) {
                    ...
                    sophia.call('alexandria_api.get_account', { account_name: membersArray[i] }, function (err, response) {
                        if (err) return callback(err);else {
                            ...
                            var data = sophia.encodeAndPack(privateKey, response.account[0].memo_key, group_operation.toObject(group_object));
                            var group_meta = {
                                sender: pk,
                            ...
                            if (i === membersArray.length - 1) {
                                ...
                                var group = new groups(retObjects);
...
sophia.addGroupParticipants = function (groupName, newMembers, admin, privateKey, callback) {
    var g_ob = groups.findOne({ 'group_name': groupName }, function (err, obj) {
        if (err) return err;else {
            console.log(obj);
            // assert.equal(obj===null,'{ operation_payloads: [ [ \'martyn\', [Object] ], [ \'matej\', [Object] ] ],\n' +
            //     '  _id: 5c5d548435a74bcc395e7d8e,\n' +
            //     '  group_name: \'tkd5D4zaqjS1q1AMtB/5Wj1xZWQ=\',\n' +
            //     '  __v: 0 }','group not found');
            // assert.equal(obj.admin === admin,true,'you are not the admin');
            if (obj) return obj;
        }
    });
    return sophia.call('alexandria_api.get_account', { account_name: g_ob.adminName }, function (err, res) {
        if (err) return callback(err);else {
            (function () {
                var admin = res.account[0];
                var group_name = sophia.suggestGroupName(g_ob.description);
                var admin_name = admin.name;
                var groupKey = sophia.createRandomKey();
                var pk = admin.memo_key;
                var membersArray = g_ob.members;
                membersArray.push(newMembers);
                var ret = [];
                    ...
                    sophia.call('alexandria_api.get_account', { account_name: membersArray[i] }, function (err, response) {
                        if (err) return callback(err);else {
                            var group_object_add = {
                                version: 1,
                                type: "add",
                                description: g_ob.description,
                                new_group_name: group_name,
                                user_list: membersArray,
                                senders_pubkey: pk,
                                new_key: [[response.account[0].memo_key, groupKey]] //check for encrypted key
                            };
