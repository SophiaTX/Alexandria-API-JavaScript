...
var Sophia = function (_EventEmitter) {
    ...
    _createClass(Sophia, [{
        ...
        value: function logError(err) {
            var message = _logging2.default.ErrorMessage(err, this.options.uri);
            _logging2.default.emit('gelf.log', message, function (err) {
        ...
        value: function _setLogger(options) {
            _logging2.default.setHost(options.logUri);
            ...
            try {
                return this.call('calculate_fee', [operation, 'SPHTX'], function (err, response) {
                    if (err) callback(err, '');else {
                        _this4.call('add_fee', [operation, response], function (err, response) {
                            if (err) _this4.logError(e);else {
                                transaction = response;
                                _this4.call('about', [operation, response], function (err, response) {
                                    if (err) callback(err, '');else {
                                        chainId = response.chain_id;
                                        _this4.call('create_simple_transaction', [transaction], function (err, response) {
                                            if (err) callback(err, '');else {
                                                ...
                                                _this4.call('broadcast_transaction', [signedTransaction], function (err, response) {
                                                    if (err) {
                                                        callback(err, '');
                                                    } else {
                                                        callback('', response);
