...
module.exports = function (sophiaAPI) {
  ...
  function estimateAccountValue(account) {
    ...
    if (!vesting_steem || !feed_price) {
      if (!gprops || !feed_price) {
        promises.push(sophiaAPI.getStateAsync("/@{username}").then(function (data) {
          gprops = data.props;
    ...
    if (!open_orders) {
      promises.push(sophiaAPI.getOpenOrdersAsync(username).then(function (open_orders) {
    ...
    if (!savings_withdraws) {
      promises.push(sophiaAPI.getSavingsWithdrawFromAsync(username).then(function (savings_withdraws) {
