...
functions.updateAccount=function(account_name,json_meta,owner,active, memo_key,private_key,callback){
    return sophia.call('update_account',[account_name, json_meta, owner, active, memo_key],(err,response)=>{
        if(err)
            callback(err,null);
        else {
            sophia.startBroadcasting(response,private_key,callback)
...
functions.deleteAccount=function(account_name,private_key,callback){
    return sophia.call('delete_account',[account_name],(err,response)=>{
        if(err)
            callback(err,'');
        else {
            sophia.startBroadcasting(response,private_key,callback)
...
functions.setVotingProxy=function(account_to_modify,proxy,private_key,callback){
    return sophia.call('set_voting_proxy',[account_to_modify, proxy],(err,response)=>{
        if(err)
            callback(err,'');
        else {
            sophia.startBroadcasting(response,private_key,callback)
 ...
 * @returns {object}
 */
functions.voteForWitness=function(accountToVoteWith, witness_to_vote_for, approve=true,private_key,callback){
    return sophia.call('vote_for_witness',[accountToVoteWith, witness_to_vote_for, approve],(err,response)=>{
        if(err)
            callback(err,'');
        else {
            sophia.startBroadcasting(response,private_key,callback);
...
functions.withdrawVesting=function(from,vesting_shares,private_key,callback){
    return sophia.call('withdraw_vesting',[from,vesting_shares],(err,response)=>{
        if(err)
            callback(err,'');
        else {
            sophia.startBroadcasting(response,private_key,callback);
...
functions.updateWitness=function(account_name, url, block_signing_key, accountCreationFee,maximumBlockSizeLimit,prizeFeeds,private_key,callback){
    ...
    return sophia.call('update_witness',[account_name, url, block_signing_key, props],(err,response)=>{
        if(err)
            callback(err,'');
        else {
            sophia.startBroadcasting(response,private_key,callback)
